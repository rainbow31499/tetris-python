# Version 0.4: Include a bank of dropped blocks
...
class Block:
    ...
    def rotatable(self, position, dropped_blocks):
        ...
        for block in rotated_block:
            ...
            if block_position[1] < 0 or block_position[0] < 0 or block_position[0] > 9 or block_position in dropped_blocks:
                result = False
        return result
    ...
    def fallable(self, position, dropped_blocks):
        ...
        for block in self.blocks:
            ...
            if block_moved[1] < 0 or block_moved in dropped_blocks:
                result = False
        return result
    ...
    def leftable(self, position, dropped_blocks):
        ...
        for block in self.blocks:
            ...
            if block_moved[0] < 0 or block_moved in dropped_blocks:
                result = False
        return result
    ...
    def rightable(self, position, dropped_blocks):
        ...
        for block in self.blocks:
            block_moved = [block[0] + position[0] + 1, block[1] + position[1]]
            if block_moved[0] > 9 or block_moved in dropped_blocks:
                result = False
        return result
...
while running:
        ...
        if time.time() - block_fall_time >= fall_period:
            if active_block.fallable(active_block_position, dropped_blocks) == True:
                ...
                for block in active_block.blocks:
                    ...
                    dropped_blocks.append(block_position)
                    dropped_blocks_colors[tuple(block_position)] = tetris_colors[block_number]
    ...
    for block in dropped_blocks:
        pygame.draw.rect(screen, dropped_blocks_colors[tuple(block)], pygame.Rect(27+30*(block[0]),597-30*(block[1]),26,26))
